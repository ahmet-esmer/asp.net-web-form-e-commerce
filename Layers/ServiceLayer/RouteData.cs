...
namespace ServiceLayer
{
    public static class RouteDataGet
    {
        ...
        public static T Value<T>(string parameterName, T defaultValue) where T : IConvertible
        {
            ...
            if (request.RequestContext.RouteData.Values[parameterName] != null &&
                !string.IsNullOrEmpty(request.RequestContext.RouteData.Values[parameterName].ToString()))
                input = request.RequestContext.RouteData.Values[parameterName];
            else
                return defaultValue;
